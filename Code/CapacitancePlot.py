...
## Read data
def PlotCV(ax1, DataArray):
    CVPath = DataArray[0]
    Filter1 = DataArray[1] if len(DataArray) > 1 else ''
    Filter2 = DataArray[2] if len(DataArray) > 2 else ''
    ...
    if i == 0:
        print("Didn't find any matching data for %s with specified filter. Exiting..."%CVPath)
        exit(1)
